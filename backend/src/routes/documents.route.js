...
router
  .route("/")
  .get(isLoggedIn, DocumentsCtrl.apiGetDocuments) // 유저별 전체 문서 조회
...
router
  .route("/:id")
  .get(isLoggedIn, DocumentsCtrl.apiGetDocumentById) // 유저별 특정 문서 1개 조회 (+ 서명 튜플)
  .delete(isLoggedIn, DocumentsCtrl.apiDeleteDocumentById); // 유저별 특정 문서 1개 삭제 (요청자(UserId1과 일치)인 경우에만)
...
router
  ...
  .put(isLoggedIn, SignaturesCtrl.apiUpdateSignature); // 유저의 서명 수정
